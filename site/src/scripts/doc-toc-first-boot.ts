	...
	if (hash && hash.length >= 2) {
		...
		function measureInd(list: Element, liNodes: Element[]) {
			...
			for (const node of liNodes) {
				const liR = node.getBoundingClientRect();
		...
		for (const link of links0) {
			const href0 = link.getAttribute('href') ?? '';
			const tid0 = href0.startsWith('#') ? href0.slice(1) : '';
		...
		const vpTop0 =
			ch0 instanceof HTMLElement ? ch0.getBoundingClientRect().bottom : 0;
		...
		for (const head of activeHeads) {
			...
			for (const cand0 of links0) {
				...
				if (liN0 && !liNodesInd.includes(liN0)) liNodesInd.push(liN0);
				break;
			}
		}
		...
		if (activeHeads.length > 0) {
			...
			for (const link of links0) {
				const rawN = (link.getAttribute('href') ?? '').replace(/^#/, '');
				if (rawN === midId0) {
					targetLk = link;
					break;
				}
			...
			if (targetLk) {
				...
				if (maxS0 > 0) {
					...
					const wm0 = window.innerHeight / 2;
					const lm0 = tr0.top + tr0.height / 2;
					let nt0 = toc.scrollTop + (lm0 - wm0);
					nt0 = Math.min(Math.max(0, nt0), maxS0);
					toc.scrollTop = nt0;
				}
			}
		} else {
