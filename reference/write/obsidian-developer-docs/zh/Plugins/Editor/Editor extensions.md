...
- 如果您想更改在阅读视图中将 Markdown 转换为 HTML 的方式，请考虑构建一个 [[Markdown post processing|Markdown post processor]]。
...
要注册编辑器扩展，请在 Obsidian 插件的 `onload` 方法中使用 [[registerEditorExtension|registerEditorExtension()]]：
...
虽然 CM6 支持多种类型的扩展，但最常见的两种是 [[View plugins]] 和 [[State fields]]。
