---
aliases: "BasesPropertyOption"
cssclasses: hide-title
---

<!-- 不要编辑此文件。它由 API Documenter 自动生成。 -->

[`BasesPropertyOption`](BasesPropertyOption)

## BasesPropertyOption 接口

允许选择属性的下拉菜单。

 1.10.0

**签名：**

```typescript
export interface BasesPropertyOption extends BasesOption 
```
**扩展：** [`BasesOption`](BasesOption)

## 属性

|  物业 |修改器 |类型 |描述 |
|  --- | --- | --- | --- |
|  [`默认？`](BasesPropertyOption/默认) |  | <code>字符串</code> | _（可选）_ 1.10.0 |
|  [`显示名称`](BasesOption/显示名称) |  | <code>字符串</code> | <p> 1.10.0</p><p>（继承自 [BasesOption](BasesOption)<!-- -->)</p> |
|  [`过滤器？`](BasesPropertyOption/过滤器) |  | <code>(prop: </code>[`BasesPropertyId`](BasesPropertyId)<code>) =>;布尔值</code> | <p>_（可选）_ 如果提供，则只有通过过滤器的属性才会包含在属性下拉列表中以供选择。</p><p> 1.10.0</p> |
|  [`key`](BasesOption/key) |  | <code>字符串</code> | <p> 1.10.0</p><p>（继承自 [BasesOption](BasesOption)<!-- -->)</p> |
|  [`占位符？`](BasesPropertyOption/占位符) |  | <code>字符串</code> | _（可选）_ 1.10.0 |
|  [`shouldHide？`](BasesOption/shouldHide) |  | <代码>() =>布尔值</code> | <p>_（可选）_ 如果提供，则在函数返回 true 时该选项将被隐藏。</p><p> 1.10.2</p><p>（继承自 [BasesOption](BasesOption)<!-- -->)</p> |
|  [`类型`](BasesPropertyOption/类型) |  | <code>'属性'</code> |  1.10.0 |

